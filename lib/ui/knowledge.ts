...
export function knowledgeGroups(s: GameState): KnowledgeGroup[] {
 return [
  ...
  {id:'people',title:'人物',entries:npcs.filter(x=>s.npcKnowledge[x.id]?.observed).map(x=>{const k=s.npcKnowledge[x.id];return {id:x.id,title:k.matched&&k.knownName?k.knownName:x.observedLabel,detail:x.observation+(k.matched&&k.knownIdentity?` 已确认：${k.knownIdentity}`:'')}})},
