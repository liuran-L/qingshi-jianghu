...
export function artGraph(s: GameState, tree: ArtTree, actions: LimitedAction[], readingReady = true) {
 ...
 const nodes=source.map(n=>{
  ...
  const reason=state==='learned'?'已投入一点。':missingPre?`先学习${artNodes.find(x=>x.id===n.prerequisite)!.title}`:!entered?'先通过实际互动取得此门技艺的入门指点。':artBlock(s,n)??(!readingReady?'先读完当前对话并等待结算。':!action?'当前处境不能投入；请回到自由行动或打开场景中的功法选项。':'可以投入一点。');
...
export function artAcquisition(tree: ArtTree) {
 const intro=tree==='step'?'先与步法导师实际交往；序章正式指点需六十分钟。':tree==='medicine'?'先与医者实际交往；序章协助辨伤需四十五分钟。':'先在剧情中帮助相应导师，再请教入门。';
