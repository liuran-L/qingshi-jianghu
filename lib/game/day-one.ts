...
export function dayOneEntry(state: GameState, npcId: string | null): LimitedAction[] {
  ...
  return [action('open-dayone', npcId === 'shen-yanqiu' ? '坐下问诊' : npcId === 'su-wantang' ? '到柜前细谈' : '与眼前人细谈', '我想把眼前的事问清楚。', 'action')];
}
export function dayOneActions(state: GameState, npcId: string | null): LimitedAction[] {
  ...
  if (state.gatePhase === 'detained' || ['questioning', 'answered', 'disputed'].includes(d.review)) {
    if (npcId === 'ma-sandao' && d.review === 'none') add('request-review', '请当值总捕头来复核（30分钟）', '请叫当值总捕头复核，扣了什么也请记明白。');
    if (npcId === 'ning-buping') {
      if (d.review === 'questioning') { add('review-confirm', '照实确认遇袭，只说记得的部分'); add('review-deny', '改口说路上不曾遇袭', '我在来青石的路上不曾遇袭。'); }
      if (d.review === 'disputed') add('review-correct', '承认刚才改了口，请他另页更正');
      if (d.review === 'answered') add('review-release', '签过复核页，领回原物（20分钟）');
    }
    return options;
  }
  ...
  if (npcId === 'ma-sandao') {
    ...
    if (state.gatePhase === 'questioning') { add('tell-amnesia', '照实说遇袭后记不起籍贯', '我叫' + state.player.name + '。遇袭后记不清籍贯，不敢乱报，只想进城治伤。'); add('tell-confused-origin', '只说记得来此投宿', '我叫' + state.player.name + '。籍贯的记忆混在一起，我只记得此行要来投宿。'); }
  ...
  if (npcId === 'su-wantang') {
    if (d.registration === 'none') { add('register-true', '在房簿写下「' + state.player.name + '」'); add('register-alias', '另报「' + aliasName(state) + '」写入房簿'); add('register-refuse', '不留姓名，也不住店'); }
  ...
  if (npcId === 'shen-yanqiu') {
    if (d.interview === 'none') { add('medical-truth', '照实说是刀刃所伤，记忆有缺'); add('medical-lie', '说成是枯枝划伤'); add('medical-refuse', '不说伤口来由'); add('medical-fee', '先问诊金与会查哪些伤处'); }
    else if (d.consent === 'none') { add('consent-exam', '请他完整验伤，稍后再定布条去留', '我同意完整验伤，布条是否留下另行决定。'); add('consent-basic', '只请他止血，不验药性'); add('refuse-exam', '不拆布条，暂且离开'); }
    else if (d.consent === 'exam' && d.cloth === 'undecided') { add('retain-cloth', '把旧布条留给医者检验'); add('keep-cloth', '自己收回布条，暂不化验'); }
    else {
  ...
  return [...options.slice(0, 5), action('close-dayone', '先谈到这里', '我先退开，看看眼前还有什么事。', 'action')];
}
...
export function resolveDayOne(state: GameState, request: InteractionRequest): RuleResolution | null {
  ...
  switch (request.actionId) {
    ...
    case 'tell-amnesia': return { ...say('“记不清便写不详。先把记得的姓名和来由说清，胡编籍贯，后头总要对簿。”'), gatePhase: 'explaining', npcStatePatch: { suspicion: state.npcStates['ma-sandao'].suspicion + 1 } };
    ...
    case 'review-confirm': return say('“原话留在前页，今日补充另记。想不起的地方写想不起，不必硬填。”', 10);
    ...
    case 'register-true': case 'register-alias': return say('“我照你亲口报的写。住店另付房钱，日后有人来问，这一页也还在。”', 0);
    ...
    case 'lu-stance': return { timeCostMinutes: 10, narration: '你徒手比出握剑姿势，重心一转，脚下便晃了半步。负剑客用筷子轻点桌角：“肩松，脚稳。先站住，再谈出手。”' };
    case 'lu-test-lie': return say('“方才脚下那一晃，我就在面前看着。试我眼力可以，别拿看过的事改口。”');
    case 'listen-guests': return { timeCostMinutes: 10, narration: '酒客们说，近来商队入城都要留名。几个人说法不同，只有“客栈留着住店名册”这一句彼此对得上。' };
    ...
    case 'pay-basic': return { timeCostMinutes: 15, moneyDelta: -3, narration: '你付了三两。医者重新压紧绷带，眼下可缓住' + (state.growth.medicine.nodes.includes('medicine-bandage') ? '二百四十' : '一百八十') + '分钟的失血；创口与沾过的药仍未处理。' };
    ...
    case 'medical-leave': return { timeCostMinutes: 0, narration: '你收好衣襟，退回候诊处。沈砚秋没有再碰你未许他检查的伤口。' };
    ...
    case 'request-medical-debt': return say('“眼下赊不了这副药。诊单我还没落笔，也没有动药柜。”');
    default: return null;
  }
}
