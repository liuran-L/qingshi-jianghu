...
export function dayTwoActions(state: GameState, npcId: string | null): LimitedAction[] {
  ...
  if (dangerConfirmed && !state.saltCase.reported && state.dayTwo.departurePlan === 'none') out.push(choice('decline-report', '先不递状，打听今夜离县的小渡', '我眼下不递状，只想问问今晚有没有离开青石的小船。'));
  if (canBroker && state.dayTwo.brokerContact === 'none') out.push(choice('accept-broker-contact', '接下只认残片的渡口纸条', '若有人只认残片、不问其余，我愿去河岸听他一句。'));
...
export function resolveDayTwo(state: GameState, request: InteractionRequest): RuleResolution | null {
  switch (request.actionId) {
    ...
    case 'decline-report': return { timeCostMinutes: 5, narration: '你没有当场递状。有人指给你一条夜里的小渡：船家只问船资与去向，不问你替谁带着什么。', discoveredLocationIds: ['dock'] };
    case 'accept-broker-contact': return { timeCostMinutes: 5, narration: '柜台边递来一张没有署名的纸条：今夜河岸，带残片来。纸上没写程守义，也没提病案与城门口供。', discoveredLocationIds: ['dock'] };
