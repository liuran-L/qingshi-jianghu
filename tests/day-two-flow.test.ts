...
void test('R04 错过货车只在满足路线资格时保留渡口出口；无资格仍是失败事实', async () => {
  ...
  let route = await prepared(); route = await click(route, 'open-daytwo'); route = await click(route, 'decline-report'); route = movePlayer(route, 'dock'); route = await waitForDeparture(route);
